...
class DataSet(tordata.Dataset):
    ...
    def __loader__(self, paths):
        ...
        for idx, data in enumerate(data_list):
            """
            print(idx, len(data), len(data[0]), len(data[0][0]), paths)
            0 125 64 64 ['/data/leo_data/CASIA-B-pkl/077/nm-01/018/018.pkl']
            """
    ...
    def __getitem__(self, idx):
        ...
        # import cv2
        # print(idx, data_list[0])
        # cv2.imwrite('./{0}.png'.format(idx), data_list[0][0])
                ...
                for typ in sorted(os.listdir(osp.join(dataset_root, lab))):
                    for vie in sorted(os.listdir(osp.join(dataset_root, lab, typ))):
                        ...
                        if seq_dirs != []:
                            ...
                            """
                            ['111', 'cl-01', '144', ['/data/leo_data/CASIA-B-pkl/111/cl-01/144/144.pkl']]
                            print([*seq_info, seq_dirs])
                            """
                        else:
                            msg_mgr.log_debug(
