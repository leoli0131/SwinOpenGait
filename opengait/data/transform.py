...
class BaseSilCuttingTransform():
    ...
    def __call__(self, x):
        ...
        """
        print(x.shape, self.cutting, cutting)
        (1070, 64, 64) None
        (1001, 64, 64) None
        """
        # x = x[..., cutting:-cutting]
...
class BaseRgbTransform():
    def __init__(self, mean=None, std=None):
        if mean is None:
            # why 0.485, 0.456, 0.406
            mean = [0.485*255, 0.456*255, 0.406*255]
        if std is None:
            #why 0.229, 0.224, 0.225 
            std = [0.229*255, 0.224*255, 0.225*255]
            # reshape parameter: array, newshape, order, subok 
            # (1,3,1,1) tuple indicates arrar 
...
class RandomRotate(object):
    ...
    def __call__(self, seq):
        if random.uniform(0, 1) >= self.prob:
            ...
            # affine 参数： src -图像， M-变换矩阵, dsize - 输出图像大小, flags - 插值方法的组合, borderMode - 边界像素模式, borderValue - 边界填充值
